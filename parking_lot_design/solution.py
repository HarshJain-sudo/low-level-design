...
This file contains the complete implementation of the Parking Lot system
...
class HourlyFeeStrategy(FeeStrategy):
    def calculate_fee(self, ticket: Ticket) -> float:
        hours = (
            (ticket.leave_datetime - ticket.issued_time).total_seconds()
            / 3600
        )
...
class ParkingManager:
    ...
    def park_vehicle(self, vehicle: Vehicle) -> Ticket:
        ...
        if not slot:
            print(
                f"No available slot for vehicle size {vehicle.size}"
            )
            return None
        ...
        print(
            f"Vehicle {vehicle.licence_number} parked at slot "
            f"{slot.parking_slot_id}"
        )
        return ticket
    ...
    def unpark_vehicle(self, ticket_id: str):
        ...
        for floor in self.parking_lot.floors:
            for slot in floor.slots:
                if (
                    slot.vehicle
                    and slot.vehicle.licence_number == ticket.licence_number
                ):
                    slot.remove_vehicle()
